...
function Task({task, onDelete, onEdit}: any) {
  ...
  return (
    <div key={task.id} className='flex items-center justify-center w-full max-w-4xl text-slate-200'>
      <div className={`${isEditing ? 'bg-[#434c5e]' : 'bg-[#3b4252]'} flex items-center justify-center select-none h-10 w-10 rounded-l-md`}>
      ...
      <div className={`${isEditing ? 'bg-[#434c5e]' : 'bg-[#3b4252]'} flex flex-col items-center justify-center gap-1 rounded-r-md w-10 h-10`}>
        <button className=''>
          {!isEditing ? 
            <img src={deleteIcon} alt="" className='w-6 h-6 select-none' onClick={() => onDelete(task.id)} /> : 
            <img src={saveIcon} alt="" className='w-6 h-6 select-none' onClick={() => setIsEditing(false)} />
          }
        </button>
      </div>
